...
BEGIN TRY
    ...
    DECLARE @id_compania        NVARCHAR(4)     =   '4826',
            ...
            @id_documento       NVARCHAR(6)     =   '123456',
            @nombre_documento   NVARCHAR(255)   =   'aprobar_pedido_vtex',
    ...
    /* =============================================
       DEDUPLICACIÓN DE PEDIDOS DEL ERP (t430)
       ============================================= */
    ...
    INSERT INTO @t430_cm_pv_docto (f430_num_docto_referencia, f430_consec_docto)
    SELECT
    ...
    /* =============================================
       ÓRDENES PROCESABLES (ESTADO 6: Pedido Por Aprobar)
       ============================================= */
    ...
    IF @@ROWCOUNT = 0
    BEGIN
        PRINT 'No hay órdenes pendientes en estado 6 para aprobar en este batch.';
        RETURN;
    END
    ...
    PRINT CONCAT('Batch procesado: ', @@ROWCOUNT, ' órdenes de aprobación actualizadas.');
...
BEGIN CATCH
    ...
    SET @msgFinal = CONCAT(
        N'[APROBAR_PEDIDO_VTEX] Error línea ', CAST(@errorLine AS NVARCHAR), N' | ',
        N'Proc: ', @errorProc, N' | ',
        N'Msg: ', @errorMsg, N' | ',
