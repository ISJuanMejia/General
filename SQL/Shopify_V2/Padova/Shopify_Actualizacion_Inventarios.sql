...
(
    ...
    f121_id_ext1_detalle        VARCHAR(20)
);
...
BEGIN TRY
    ...
    SELECT DISTINCT
        ...
        barcode =
            ...
            END,
        bodega = f150_id,
        id_ext1_detalle = f121_id_ext1_detalle
    ...
    SELECT
        ...
        barcode             =   JSON_VALUE(v.variante_obj, '$.sku'),
        cantidad            =   ISNULL(CONVERT(INT, iss.cantidad), 0),
        id_location         =   i.id_location,
        bodega              =   b.bodega_erp,
        id_variante         =   v.id_variante
    FROM variantes v
        INNER JOIN @inventarios iss
            ON
                JSON_VALUE(v.variante_obj, '$.sku') = iss.barcode
                AND
                (
                    CASE JSON_VALUE(v.variante_obj, '$.title')
        ...
        INNER JOIN inventario_bodega_ecommerce i
            ON 
                I.inventory_item_id = JSON_VALUE(v.variante_obj, '$.inventory_item_id')
    ...
    USING @inventarios_siesa_shopify AS source
        ON
            ...
            AND 
            JSON_VALUE(target.inventario_obj, '$.location_id') = source.id_location
