...
DECLARE @id_criterio_mayor      VARCHAR(4)      = '01'; 
DECLARE @id_cia_erp                 INT         = 7;
...
BEGIN TRY
    ...
    ----------------------------------------------------------------------- */
    INSERT INTO @t126_mc_items_precios (f126_rowid_item, f126_precio, f126_fecha_activacion)
    EXEC('
        SELECT f126_rowid_item, f126_precio, f126_fecha_activacion
        FROM OPENROWSET(
            ''sqlncli'',
            ''' + @conexion + ''',
            ''
                SELECT
                    f126_rowid_item, f126_precio, f126_fecha_activacion
                FROM (
                    SELECT f126_rowid_item, f126_precio, f126_fecha_activacion,
                        ROW_NUMBER() OVER (
                            PARTITION BY
    ...
    SELECT DISTINCT
        ...
        precio_obj = (
            SELECT
                ...
                t126.f126_precio   AS [variant.compare_at_price]
    ...
    FROM @t126_mc_items_precios t126
        ...
        INNER JOIN variantes v
            ON
                v.sku_erp = t120.f120_id
                        + CASE WHEN t121.f121_id_ext1_detalle IS NOT NULL THEN '-' + t121.f121_id_ext1_detalle ELSE '-0' END
                        + CASE WHEN t121.f121_id_ext2_detalle IS NOT NULL THEN '-' + t121.f121_id_ext2_detalle ELSE '-0' END
    WHERE
        t120.f120_rowid IS NOT NULL
        AND 
        t120.f120_id IS NOT NULL
        AND 
        EXISTS (
    ...
    ----------------------------------------------------------------------- */
    MERGE INTO [shopify-colombia-womder].dbo.precios AS TARGET
    USING (
        SELECT
            id_variante,
            sku_erp,
