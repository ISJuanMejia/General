...
BEGIN TRY
    ...
    INSERT INTO @t125_mc_items_criterios
    EXEC('
        SELECT
            f125_rowid_item
        FROM OPENROWSET(
    ...
    SELECT DISTINCT
        ...
        barcode =   f120_id + 
                    CASE 
                        WHEN f121_id_ext1_detalle IS NOT NULL 
                            THEN '-' + f121_id_ext1_detalle 
                        ELSE '' 
                    END + 
                    CASE 
                        WHEN f121_id_ext2_detalle IS NOT NULL 
                            THEN '-' + f121_id_ext2_detalle 
                        ELSE '' 
                    END,
        bodega  =   f150_id
    FROM @t120_mc_items
