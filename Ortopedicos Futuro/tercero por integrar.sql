...
/*
    ...
    *   importante resaltar que estos parámetros deben ser ajustados acorde a las necesidades particulares de cada caso, 
    *   considerando la configuración y categorización establecida en el destino final para la identificación de la 
    *   compañía, el sistema y el documento, así como el nombre del documento a integrar.
*/
DECLARE @id_compania        NVARCHAR(4)     =   '4826',
        @id_sistema         NVARCHAR(1)     =   '2',
        @id_documento       NVARCHAR(6)     =   '227300',
        @nombre_documento   NVARCHAR(255)   =   'TERCERO_CLIENTE_VTEX';

/*
    *   Endpoint: URL del endpoint al cual se enviará la información formateada y estructurada acorde a las necesidades 
    *   del destino final. Es importante resaltar que este endpoint debe ser ajustado acorde a las necesidades 
    *   particulares de cada caso, considerando los diferentes escenarios y casos de uso que se puedan presentar en la 
    *   operación real del conector, para asegurar una integración exitosa y sin contratiempos. Además, es fundamental 
    *   verificar que el endpoint esté correctamente configurado y sea accesible desde el entorno donde se ejecutará el 
    *   conector, para evitar posibles errores de conexión o de envío de datos.
*/
    ...
    ELSE JSON_VALUE(orden_obj_origen, '$.clientProfileData.email') end AS F015_EMAIL,
            ...
            JSON_QUERY((
                    ...
                    FROM(
                        -- - naturales
                        SELECT
                        JSON_VALUE(orden_obj_origen, '$.clientProfileData.document')AS f200_id, 'ID PROCEDENCIA FE2.1' AS 'f753_id_grupo_entidad', 'EUNOECO036' AS 'f753_id_entidad', 'co036_id_procedencia_org' AS 'f753_id_atributo', 'MUNOECO043' AS 'f753_id_maestro', '10' AS 'f753_id_maestro_detalle')AS EntidadDinamicaTercero
